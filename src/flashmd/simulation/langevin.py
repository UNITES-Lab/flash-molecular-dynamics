...
class LangevinSimulation(_Simulation):
    ...
    @nvtx.annotate("LangevinSimulation.timestep", color="cyan")
    def timestep(
        ...
        """
        with nvtx.annotate("BAOAB_data_load", color="gray"):
            v_old = data[VELOCITY_KEY]
            masses = data[MASS_KEY]
            x_old = data[POSITIONS_KEY]

        # Log initial state shapes
        if self.print_shape and self._shape_log_file and self._shape_log_step_count <= self.print_shape_steps:
            self._shape_log_file.write("[Langevin timestep] BAOAB Integrator:\n")
            self._log_shape("  x_old (positions)", x_old)
            self._log_shape("  v_old (velocities)", v_old)
            self._log_shape("  masses", masses)
            self._log_shape("  forces (input)", forces)
            self._log_shape("  beta_mass_ratio", self.beta_mass_ratio)

        # B (first velocity half-step)
        with nvtx.annotate("BAOAB_B1_velocity_halfstep", color="red"):
            v_new = v_old + 0.5 * self.dt * forces / masses[:, None]

        # A (first position half-step)
        with nvtx.annotate("BAOAB_A1_position_halfstep", color="orange"):
            x_new = x_old + v_new * self.dt * 0.5

        # O (stochastic velocity update / noise)
        with nvtx.annotate("BAOAB_O_stochastic_velocity", color="yellow"):
        ...
        # Log noise shape
        if self.print_shape and self._shape_log_file and self._shape_log_step_count <= self.print_shape_steps:
            self._log_shape("  noise (O step)", noise)

        # A (second position half-step)
        with nvtx.annotate("BAOAB_A2_position_halfstep", color="orange"):
            x_new = x_new + v_new * self.dt * 0.5
            data[POSITIONS_KEY] = x_new

        # Log position after A steps
        if self.print_shape and self._shape_log_file and self._shape_log_step_count <= self.print_shape_steps:
            self._log_shape("  x_new (after A1-O-A2)", x_new)

        # Force calculation (most expensive part)
        with nvtx.annotate("BAOAB_force_calculation", color="green"):
            potential, forces = self.calculate_potential_and_forces(data)

        # B (second velocity half-step)
        with nvtx.annotate("BAOAB_B2_velocity_halfstep", color="red"):
            v_new = v_new + 0.5 * self.dt * forces / masses[:, None]
            data[VELOCITY_KEY] = v_new

        # Log final state
        if self.print_shape and self._shape_log_file and self._shape_log_step_count <= self.print_shape_steps:
            self._log_shape("  v_new (final)", v_new)
            self._log_shape("  potential (output)", potential)
            self._log_shape("  forces (output)", forces)
...
class OverdampedSimulation(_Simulation):
    ...
    @nvtx.annotate("OverdampedSimulation.timestep", color="cyan")
    def timestep(
        self, data: AtomicData, forces: torch.Tensor
    ) -> Tuple[AtomicData, torch.Tensor, torch.Tensor]:
        ...
        potential:
            potential evaluated at t+1
        """
        with nvtx.annotate("overdamped_data_load", color="gray"):
            x_old = data[POSITIONS_KEY]

        with nvtx.annotate("overdamped_noise_generation", color="yellow"):
            # Use pre-allocated buffer and fill in-place to avoid allocation
            self._noise_buffer.normal_(generator=self.rng)
            noise = self._noise_buffer

        with nvtx.annotate("overdamped_position_update", color="orange"):
            x_new = (
                x_old.detach()
                + forces * self._dtau
        ...
        with nvtx.annotate("overdamped_force_calculation", color="green"):
            potential, forces = self.calculate_potential_and_forces(data)
