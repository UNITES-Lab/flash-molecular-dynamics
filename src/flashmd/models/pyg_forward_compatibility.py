...
import torch
import flashmd.models.schnet
from flashmd.models.schnet import CFConv, SchNet
from flashmd.models.gradients import SumOut, GradientsOut
...
def get_refreshed_cfconv_layer(old_cfconv: CFConv):
    ...
    # to work
    flashmd.models.schnet.Tensor = torch.Tensor
    # extract init args
...
def refresh_module_(
    ...
    """In-place refresh all cfconv_layers in a torch.nn.Module
    that possibly contains a SchNet inside.
    See `get_refreshed_cfconv_layer` for what this refreshing does.
    ...
    model_type (SchNet):
        the model type to be refreshed.
    verbose (bool):
    ...
    for model in all_models:
        if model_type == SchNet:
            ...
            raise NotImplementedError(
                f"refresh_module only supports SchNet, "
...
def load_and_adapt_old_checkpoint(f, **kwargs):
    ...
    adapts SchNet models to make them usable.
